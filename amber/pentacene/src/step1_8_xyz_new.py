...
def main_process(args):
    auto_dir = f'/vol0303/data/hp260444/Working/fugaku/amber/pentacene/{args.auto_dir}'
    os.makedirs(auto_dir, exist_ok=True)
    os.makedirs(os.path.join(auto_dir,'amber'), exist_ok=True)
    os.makedirs(os.path.join(auto_dir,'gaussview'), exist_ok=True)
    amber_path=os.path.join(auto_dir,'amber')
    shutil.copy(f'/vol0303/data/hp260444/Working/fugaku/amber/pentacene/src/FF_calc.in',amber_path)
    shutil.copy(f'/vol0303/data/hp260444/Working/fugaku/amber/pentacene/monomer/{args.monomer_name}.frcmod',amber_path)
    
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):
        df=pd.DataFrame(columns=['theta','a','b','E','E1','E2','E3','status'])
        df.to_csv(auto_csv_path,index=False)
        
    auto_csv_path1 = os.path.join(auto_dir,'step1_1.csv')
    if not os.path.exists(auto_csv_path1):
        df1=pd.DataFrame(columns=['theta','a','E1','status','file_name'])
    ...
    if not os.path.exists(auto_csv_path2):
        df2=pd.DataFrame(columns=['theta','b','E2','status','file_name'])
    ...
    if not os.path.exists(auto_csv_path3):
        df3=pd.DataFrame(columns=['theta','a','b','E3','status','file_name'])
        df3.to_csv(auto_csv_path3,index=False)                

    mono_file=f'/vol0303/data/hp260444/Working/fugaku/amber/pentacene/monomer/{args.monomer_name}.out'
