...
def main_process(args):
    auto_dir = f'/home/ohno/Working/fugaku/amber/BXBX/{args.auto_dir}'
    os.makedirs(auto_dir, exist_ok=True)
    os.makedirs(os.path.join(auto_dir,'amber'), exist_ok=True)
    os.makedirs(os.path.join(auto_dir,'gaussview'), exist_ok=True)
    amber_path=os.path.join(auto_dir,'amber')
    shutil.copy(f'/home/ohno/Working/fugaku/amber/BXBX/src/FF_calc.in',amber_path)
    shutil.copy(f'/home/ohno/Working/fugaku/amber/BXBX/monomer/{args.monomer_name}.frcmod',amber_path)
    
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):
        df=pd.DataFrame(columns=['theta','A2','a','b','z','E','E1','E2','E3','E4','status'])
    ...
    if not os.path.exists(auto_csv_path4):
        df4=pd.DataFrame(columns=['theta','A2','a','b','z','E4','status','file_name'])
        df4.to_csv(auto_csv_path4,index=False)                

    mono_file=f'/home/ohno/Working/fugaku/amber/BXBX/monomer/{args.monomer_name}_mono.out'
...
def listen(auto_dir,monomer_name,E_mono,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx, row in df_prg_4.iterrows():
        ...
        log_filepath4 = os.path.join(*[auto_dir,'gaussian',file_name4])
        ...
        if len(E_list4)!=1 :##get Eの長さは計算した分子の数
            ...
            E4 = round(float(E_list4[0]) - 2 * E_mono, 4)##8分子に向けてep1,ep2作成　ep1:b ep2:a
