##tetracene層内計算
import os
os.environ['HOME'] ='/home/u15256'
import pandas as pd
from make_8_new import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path1):        
        df_E_1 = pd.DataFrame(columns = ['theta','A2','a','E1','status','file_name'])##いじる
    ...
    if not os.path.exists(auto_csv_path2):        
        df_E_2 = pd.DataFrame(columns = ['theta','A2','b','z','E2','status','file_name'])##いじる
    ...
    if not os.path.exists(auto_csv_path3):        
        df_E_3 = pd.DataFrame(columns = ['theta','A2','a','b','z','E3','E4','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,max_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    if len_qw_1 > 0 and margin > 0:# 進行中ジョブのマシンタイプをカウント
        for index, row in df_qw_1.iterrows():
            ...
            file_name = exec_gjf(auto_dir, monomer_name, {**params_dict}, structure_type=1, isTest=isTest)# ジョブの実行 structure type
    ...
    if len_qw_2 > 0 and margin > 0:# 進行中ジョブのマシンタイプをカウント
        for index, row in df_qw_2.iterrows():
            ...
            file_name = exec_gjf(auto_dir, monomer_name, {**params_dict},structure_type=2,isTest=isTest)# ジョブの実行
            len_queue += 1;margin -= 1
    ...
    if len_qw_3 > 0 and margin > 0:# 進行中ジョブのマシンタイプをカウント
        for index, row in df_qw_3.iterrows():
            ...
            file_name = exec_gjf(auto_dir, monomer_name, {**params_dict}, structure_type=3,isTest=isTest)# ジョブの実行
            len_queue += 1;margin -= 1
        ...
        for i in range(len(dict_matrix)):
            ...
            if not(alreadyCalculated):##ここで各点について1~3を実行しつつ余ったものをqwにぶち込む
                ...
                if len(df_sub_1) == 0:#(len(df_done_1_)>=1) or(len(df_inpr_1_)>=1) or (len(df_qw_1_)>=1):
                    ...
                    if isAvailable:
                        file_name = exec_gjf(auto_dir, monomer_name, {**params_dict1},  structure_type=1,isTest=isTest);len_queue +=1
                        ...
                        df_newline_1 = pd.Series({**params_dict1,'E1':0.,'status':'qw','file_name':file_name})
                        df_E_new_1=pd.concat([df_E_1,df_newline_1.to_frame().T],axis=0,ignore_index=True);df_E_new_1.to_csv(auto_csv_1,index=False)
                        #time.sleep(1)
                ...
                if len(df_sub_2) == 0:#(len(df_done_1_)>=1) or(len(df_inpr_1_)>=1) or (len(df_qw_1_)>=1):
                    ...
                    isAvailable = len_queue < max_nodes
                    if isAvailable:
                        file_name = exec_gjf(auto_dir, monomer_name, {**params_dict2}, structure_type=2,isTest=isTest);len_queue += 1
                        ...
                        df_newline_2 = pd.Series({**params_dict2,'E2':0.,'status':'qw','file_name':file_name})
                        df_E_new_2=pd.concat([df_E_2,df_newline_2.to_frame().T],axis=0,ignore_index=True);df_E_new_2.to_csv(auto_csv_2,index=False)
                ...
                if len(df_sub_3) == 0:#(len(df_done_1_)>=1) or(len(df_inpr_1_)>=1) or (len(df_qw_1_)>=1):
                    ...
                    isAvailable = len_queue < max_nodes
                    if isAvailable:
                        file_name = exec_gjf(auto_dir, monomer_name, {**params_dict3},  structure_type=3,isTest=isTest);len_queue +=1
                        df_newline_3 = pd.Series({**params_dict3,'E3':0.,'E4':0.,'status':'InProgress','file_name':file_name})
                        ...
                        df_newline_3 = pd.Series({**params_dict3,'E3':0.,'E4':0.,'status':'qw','file_name':file_name})
                        df_E_new_3=pd.concat([df_E_3,df_newline_3.to_frame().T],axis=0,ignore_index=True);df_E_new_3.to_csv(auto_csv_3,index=False)
